 ...
 * Version: 0.1-5
 *
 * ceeboo 2006, 2007, 2008, 2012, 2024
 *
...
SEXP R_transpose_ngCMatrix(SEXP x) {
  ...
  PROTECT(r = NEW_OBJECT(PROTECT(MAKE_CLASS("ngCMatrix"))));
...
SEXP R_crosstab_ngCMatrix(SEXP x, SEXP y, SEXP t) {
  ...
  if (!isNull(d1) || !isNull(d2)) {
    setAttrib(r, R_DimNamesSymbol, PROTECT(ix = allocVector(VECSXP, 2)));
    UNPROTECT(1);
    SET_VECTOR_ELT(ix, 0, d1);
    SET_VECTOR_ELT(ix, 1, d2);
    if (!isNull(n1) || !isNull(n2)) {
      setAttrib(ix, R_NamesSymbol, PROTECT(iy = allocVector(STRSXP, 2)));
      UNPROTECT(1);
...
SEXP R_colSubset_ngCMatrix(SEXP x, SEXP s) {
  ...
  PROTECT(r = NEW_OBJECT(PROTECT(MAKE_CLASS(inherits(x, "ngCMatrix") ? "ngCMatrix" : "sgCMatrix"))));
...
SEXP R_asList_ngCMatrix(SEXP x, SEXP d) {
  ...
  for (i = 1; i < LENGTH(px); i++) {
    l = INTEGER(px)[i];
    n = l-f;
    SET_VECTOR_ELT(r, i-1, PROTECT(t = allocVector((isNull(d))?INTSXP:TYPEOF(d), n)));
    UNPROTECT(1);
    m = 0;
    for (k = f; k < l; k++) {
      j = INTEGER(ix)[k];
...
SEXP R_recode_ngCMatrix(SEXP x, SEXP s) {
  ...
  PROTECT(r = NEW_OBJECT(PROTECT(MAKE_CLASS(c ? "ngCMatrix" : "sgCMatrix"))));
